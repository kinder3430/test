define(['module'], function (module) {
    ...
    var uploadTool = function uploadTool() {};
    ...
    uploadTool.prototype.init = function (mainId) {
        ...
        // 上传设置
        var uploadOpts = {
            url: GV.actionRoot + 'fileuplaod.up', // 上传链接
            data: {} // 上传参数
        };

        // 选择文件时触发
        var _onSelectFile = function _onSelectFile(evt) {
            // 所选的文件
        ...
        // 创建一个预览图像
        var _createPreview = function _createPreview($this /**jquery object*/, file /**File*/) {
        ...
        // 上传函数
        var _uploadFile = function _uploadFile($this /**jquery object*/, file /**File*/) {
        ...
        $close.on('click', function () {
            $(this).hide().siblings('.fileApi-btn').show().siblings('.fileApi-preview').children('img').attr('src', '').hide().parents('.fileApi-one-img')[0].className = 'fileApi-one-img';
        ...
        $txtIpt.on('change', function () {
            ...
            if (!val) {
                $img.hide();
                $this.siblings('.fileApi-btn').show().siblings('.fileApi-close').hide();
            } else {
                $img.show();
                $this.siblings('.fileApi-btn').hide().siblings('.fileApi-close').show();
            }
        });
    };

define(['module', '../../customize/util'], function (module, _util) {
    'use strict';
    ...
    function sidebarInit() {
        ...
        $tabs.tabs({
            fit: true,
            onClose: function onClose(title, index) {
            ...
            onSelect: function onSelect(title, index) {
                var tabsData = $.cookie('tabs') || '{}',
                    tabsDataObj = JSON.parse(tabsData);
    ...
    function updateMenu(flag) {
        $.ajax({
            type: "GET",
            url: _util2.default.getInterface(GV.menuAction),
            dataType: 'json',
                ...
                if (data.ret == 1) {
                    ...
                    for (var i = 0, navLen = nav.length; i < navLen; i++) {
                        ...
                        for (var j = 0, menusLen = menus.length; j < menusLen; j++) {
                            ...
                            $menus.append('<li><p class="menus-item" ' + 'data-href="' + path + '" ' + 'data-title="' + title + '" ' + 'data-script="' + script + '" ' + 'title="' + title + '"><span>' + title + '</span></p></li>');
                        }
                        $module.append($menus);
